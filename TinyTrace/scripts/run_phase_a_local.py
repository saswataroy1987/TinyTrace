...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "End-to-end local TinyTrace Phase-A launcher. It reuses an existing "
            "virtualenv by default, installs dependencies when needed, verifies "
            "CUDA visibility, and starts the v5 warm-start run."
        )
    )
    parser.add_argument(
        "--dataset-root",
        type=Path,
        required=True,
        help="Local copy of TinyTrace_kaggle_dataset_ready.",
    )
    parser.add_argument(
        "--work-root",
        type=Path,
        default=REPO_ROOT / "phase_a_v5_local_run",
        help="Writable local run directory.",
    )
    parser.add_argument(
        "--venv",
        type=Path,
        default=REPO_ROOT / ".venv-phase-a",
...
def _install_runtime(args: argparse.Namespace, venv_python: Path, venv_pip: Path) -> None:
    ...
    if args.device.startswith("cuda"):
        ...
        _run([str(venv_pip), "install", "torch", "torchvision", "torchaudio"])
    _run(
        [
            str(venv_python),
            str(PROJECT_ROOT / "scripts" / "setup_kaggle_env.py"),
            "--dataset-root",
...
def main() -> None:
    ...
    command = [
        str(venv_python),
        str(PROJECT_ROOT / "scripts" / "run_phase_a_kaggle.py"),
        "--dataset-root",
        str(dataset_root),
        "--work-root",
        str(work_root),
    ...
    print("Starting TinyTrace Phase A local run:", flush=True)
